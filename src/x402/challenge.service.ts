...
export function createChallengeService(deps: {
  ...
  const { challengeSecret, challengeTtlSeconds, merchantAddress, paymentChain, paymentAsset } = deps;

  return {
    generateChallenge: async function (request: ChatCompletionRequest, estimatedCost: string): Promise<PaymentRequirements> {
      ...
      const payload: ChallengePayload = {
        ...
        expires_at: new Date(Date.now() + challengeTtlSeconds * 1000).toISOString(),
      };

      const { createHmac } = await import("crypto");
      const encoder = new TextEncoder();
      const data = encoder.encode(JSON.stringify(payload));
      const signature = createHmac("sha256", challengeSecret).update(data).digest("base64url");
    ...
    verifyChallenge: async function (challengeToken: string, requestBody: ChatCompletionRequest): Promise<ChallengePayload> {
      const [signaturePart, payloadPart] = challengeToken.split(".");
      if (!signaturePart || !payloadPart) throw new Error("Invalid challenge token format");
      ...
      try {
        ...
        catch { throw new Error("Invalid challenge token format"); }

        const { createHmac, timingSafeEqual } = await import("crypto");
        const encoder = new TextEncoder();
        const data = encoder.encode(payloadString);
        const expectedSignature = createHmac("sha256", challengeSecret).update(data).digest();
        ...
        if (recomputedHash !== payload.request_hash) throw new RequestHashMismatchError();

        return payload;
      } catch (error) {
        if (error instanceof ChallengeExpiredError || error instanceof RequestHashMismatchError) throw error;
        throw error;
      }
    },
    ...
    computeRequestHash: function (body: ChatCompletionRequest): string {
      ...
      for (const key of sortedKeys) objWithSortedKeys[key] = (body as any)[key];
