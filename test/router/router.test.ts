...
describe('RouterService', () => {
  ...
  it('route should throw Not implemented', async () => {
    const request = {
      model: 'gpt-4',
      messages: [{ role: 'user' as const, content: 'hello' }],
    };
    await expect(service.route(request, 'manual')).rejects.toThrow('Not implemented');
  });
});
...
describe('FallbackService', () => {
  ...
  it('should return first successful result', async () => {
    ...
    const result = await service.executeWithFallback(['model-a', 'model-b'], executeFn);
