...
import type { UpstreamResponse, ProviderAdapter } from "../../src/provider/types.js";
...
export const DEFAULT_SIMULATION_CONFIG = {
  providers: [
    { name: "openai", models: ["gpt-4o", "gpt-4o-mini"], latencyMs: { min: 100, max: 500 }, failureRate: 0.01, rateLimitRPS: 100 },
    { name: "anthropic", models: ["claude-3-opus", "claude-3-sonnet"], latencyMs: { min: 150, max: 600 }, failureRate: 0.02, rateLimitRPS: 80 },
...
export function createSimulatedAdapter(
  ...
  return {
    ...
    execute: async (request: ChatCompletionRequest, modelId: string): Promise<UpstreamResponse> => {
      ...
      const latencyMs = Math.floor(Math.random() * (provider.latencyMs.max - provider.latencyMs.min + 1)) + provider.latencyMs.min;
      await new Promise((resolve) => setTimeout(resolve, latencyMs));
      const promptTokens = Math.floor(request.messages.reduce((acc, m) => acc + m.content.length / 4, 0));
      const completionTokens = Math.floor(Math.random() * 200) + 50;
      return {
        model_used: `${provider.name}/${modelId || provider.models[0]}`,
        choices: [{ index: 0, message: { role: "assistant", content: `[Simulated ${provider.name} response]` }, finish_reason: "stop" }],
        usage: { prompt_tokens: promptTokens, completion_tokens: completionTokens, total_tokens: promptTokens + completionTokens },
        latency_ms: latencyMs,
      };
    },
    healthCheck: async () => Math.random() > provider.failureRate,
  };
}
...
export function createSimulatedVerifyService(options?: {
  ...
  const latency = options?.latencyMs ?? { min: 50, max: 150 };
  const failure = options?.failureRate ?? 0;
  const insufficient = options?.insufficientPaymentRate ?? 0;
  return {
    verifyPayment: async (proof: PaymentProof, challenge: { amount: string }): Promise<VerificationResult> => {
      const delay = Math.floor(Math.random() * (latency.max - latency.min + 1)) + latency.min;
      await new Promise((resolve) => setTimeout(resolve, delay));
      if (Math.random() < failure) throw new PaymentVerificationFailedError("Simulated verification failure");
      if (Math.random() < insufficient) throw new InsufficientPaymentError(challenge.amount, "0.0001");
      return { verified: true, payer_address: proof.payer_address, amount: challenge.amount };
    },
    verifyPaymentV2: async () => {
      const delay = Math.floor(Math.random() * (latency.max - latency.min + 1)) + latency.min;
      await new Promise((resolve) => setTimeout(resolve, delay));
      if (Math.random() < failure) throw new PaymentVerificationFailedError("Simulated verification failure");
      if (Math.random() < insufficient) throw new InsufficientPaymentError("0.001", "0.0001");
...
export function createSimulatedReplayService() {
  ...
  return {
    checkAndMark: async (hash: string, ttlSeconds: number): Promise<boolean> => {
      await new Promise((r) => setTimeout(r, Math.random() * 10));
      if (seenHashes.has(hash)) throw new PaymentReplayedError();
      seenHashes.add(hash);
      setTimeout(() => seenHashes.delete(hash), ttlSeconds * 1000);
      return true;
    },
    checkIdempotency: async (key: string) => { await new Promise((r) => setTimeout(r, Math.random() * 5)); return idempotencyStore.get(key) || null; },
    saveIdempotency: async (key: string, response: unknown, ttlSeconds: number) => {
      await new Promise((r) => setTimeout(r, Math.random() * 10));
      idempotencyStore.set(key, response);
      setTimeout(() => idempotencyStore.delete(key), ttlSeconds * 1000);
    },
  };
}

export function createSimulatedRouterService(providers: SimulatedProvider[]) {
  const adapters = new Map(providers.map((p) => [p.name, createSimulatedAdapter(p)]));
  return {
    route: async (request: ChatCompletionRequest): Promise<{ decision: RouteDecision; response: UpstreamResponse }> => {
      ...
      const response = await adapter.execute(request, modelName || request.model);
      const decision: RouteDecision = {
        selected_model: request.model, fallback_chain: [], score_summary: "manual selection",
...
export function buildSimulationEnvironment(config = DEFAULT_SIMULATION_CONFIG): {
  ...
  metrics: { getRequestCount: () => number; getErrorCount: () => number; getAverageLatency: () => number; reset: () => void };
} {
  ...
  for (const provider of config.providers) {
    ...
    for (const model of provider.models) {
      providerRegistry.register(`${provider.name}/${model}`, adapter, {
        input_usd_per_token: "0.00001", output_usd_per_token: "0.00003", effective_at: new Date().toISOString(),
      });
    }
  }
  ...
  const originalRoute = services.routerService.route.bind(services.routerService);
  services.routerService.route = async (request) => {
    const startTime = Date.now();
    requestCount++;
    try { const result = await originalRoute(request); totalLatency += Date.now() - startTime; return result; }
    catch (error) { errorCount++; throw error; }
  };
  ...
  return {
    app, services,
    metrics: {
      getRequestCount: () => requestCount, getErrorCount: () => errorCount,
      getAverageLatency: () => (requestCount > 0 ? totalLatency / requestCount : 0),
      reset: () => { requestCount = 0; errorCount = 0; totalLatency = 0; },
    },
  };
}
